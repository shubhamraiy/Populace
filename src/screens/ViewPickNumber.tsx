...
const ViewPickNumber: React.FC<Props> = props => {
  ...
  const getSpecificPickNumbersList = async () => {
    const response: any = await ApiServices.post(
      ApiEndPoint.specificPickNumbersList,
      JSON.stringify({
  ...
  const onDelete = async (id: any) => {
    const response: any = await ApiServices.post(
      ApiEndPoint.specificNumberDelete,
      JSON.stringify({
  ...
  return (
    <MySafeArea componentId={props.componentId} isScroll={true}>
      ...
      {result?.length > 0 &&
        _renderRoundBtn(true, result[0]?.number, result[0]?._id)}
      {result?.length > 1 &&
        _renderRoundBtn(false, result[1]?.number, result[1]?._id)}
      {result?.length > 2 &&
        _renderRoundBtn(false, result[2]?.number, result[2]?._id)}
    </MySafeArea>
  );
};
