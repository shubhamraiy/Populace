...
const SweepPickNumber: React.FC<Props> = props => {
  ...
  // var getSec = Utils.getSeconds("2023-02-21T15:30:00.000Z");
  // var getHrs = Utils.toHHMMSS(result?.timerDate);
  // var getTime= Utils.intoAMPM(result?.timerDate);
  // console.log(getHrs);
  ...
  const remainingTimer = () => {
    ...
    let mint = minute ?? '00';
    let sec = second ?? '00';
    let timerD = day + 'd : ' + hrs + 'h : ' + mint + 'm : ' + sec + 's';
    let timer = hrs + 'h : ' + mint + 'm : ' + sec + 's';
    if (day > 0) {
      return timerD;
    } else {
      return timer;
  ...
  const resultTimer = () => {
    ...
    let mint = minute1 ?? '00';
    let sec = second1 ?? '00';
    let scheduledTime = result?.scheduledTime ?? '00:00';
    let timerD = day1 + ' : ' + hrs + ' : ' + mint + ' : ' + sec;
    let timer = hrs + ' : ' + mint + ' : ' + sec;
    if (selectedData.length > 0) {
      if (day1 > 0) {
        return timerD;
      } else {
  ...
  const _renderNumbersLast = (list: any, type: number) => {
    return (
      <View style={styles.rowContainer}>
        <CircleView
          marginStart={0}
          title={list?.length > 0 ? list[0] : ''}
          backgroundColor={color.royalBlue}
        />
        <CircleView
          title={list?.length > 1 ? list[1] : ''}
          backgroundColor={color.royalBlue}
        />
        <CircleView
          title={list?.length > 2 ? list[2] : ''}
          backgroundColor={color.royalBlue}
        />
        <CircleView
          title={list?.length > 3 ? list[3] : ''}
          backgroundColor={color.royalBlue}
        />
        <CircleView
          title={list?.length > 4 ? list[4] : ''}
          backgroundColor={color.royalBlue}
        />
        <CircleView
          title={list?.length > 4 ? list[4] : ''}
  ...
  return (
    <MySafeArea componentId={props.componentId} isScroll={true} padding={20}>
      ...
      <Text style={styles.tvTime}>{remainingTimer()}</Text>
      {currentUser?.subscription?.productName >= 'Tier_1' &&
        _renderNumbers(numberOne, 1)}
      {currentUser?.subscription?.productName >= 'Tier_2' &&
        _renderNumbers(numberTwo, 2)}
      {currentUser?.subscription?.productName >= 'Tier_3' &&
        _renderNumbers(numberThree, 3)}
      ...
      <Text style={styles.tvTimeOut}>{resultTimer()}</Text>

      <Text style={styles.tvPickNumber}>Picked Numbers</Text>
      {selectedData.length > 0 && (
        <>
          {currentUser?.subscription?.productName >= 'Tier_1' &&
            _renderNumbersLast(lnumberOne, 1)}
          {currentUser?.subscription?.productName >= 'Tier_2' &&
            _renderNumbersLast(lnumberTwo, 2)}
          {currentUser?.subscription?.productName >= 'Tier_3' &&
            _renderNumbersLast(lnumberThree, 3)}
        </>
      )}
    </MySafeArea>
  );
